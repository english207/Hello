...
public class Acceptor
{
	...
	public synchronized PrepareResult onPrepare(Proposal szProposal)
	{
		...
		switch (status)
		{
			...
			case ACCEPTED:
				//	如果是同一个提案，只是序列号增大
				//	批准提案，更新序列号。
				if (promisedProposal.getId()<szProposal.getId() && promisedProposal.getValue().equals(szProposal.getValue()))
				{
	...
	public synchronized CommitResult onCommit(Proposal szProposal)
	{
		...
		switch (status)
		{
			...
			case ACCEPTED:
				//	同一提案，序列号较大，接受
				if	(szProposal.getId() > acceptedProposal.getId() && szProposal.getValue().equals(acceptedProposal.getValue())){
